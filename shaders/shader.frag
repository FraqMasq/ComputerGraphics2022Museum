...
vec3 compute_Color(vec3 N, vec3 V, vec3 Cd, vec3 Cs, float gamma) {
	...
	for(int i=0; i<n_spot; i++){
		...
		fact2 = clamp((dot(normalize(gubo.spotPositions[i] - fragPos), gubo.spotDirections[i]) - gubo.coneInOutDecayExp.x) / (gubo.coneInOutDecayExp.y - gubo.coneInOutDecayExp.x),
	                    0,
						1);
		scol[i] = fact1 * fact2;
	}
