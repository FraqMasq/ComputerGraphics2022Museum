...
protected:
    ...
    void localInit() {
        ...
        P2.init(this, "shaders/planet_vert.spv", "shaders/planet_frag.spv", {&DSLGlobal, &DSLObj});
        ...
        for (int j = 0; j < componentsVector2.size(); j++) {
            ...
            componentsVector2[j].DS.init(this, &DSLObj, {
                    /*
                    // the second parameter, is a pointer to the Uniform Set Layout of this set
                    // the last parameter is an array, with one element per binding of the set.
                    // first  elmenet : the binding number
    ...
    void populateCommandBuffer(VkCommandBuffer commandBuffer, int currentImage) {
        ...
        vkCmdBindDescriptorSets(commandBuffer,
                                ...
                                curScene == 0 ? P1.pipelineLayout : P2.pipelineLayout, 0, 1,
                                curScene == 0 ? &DS_GLOBAL.descriptorSets[currentImage]
                                              : &DS_GLOBAL2.descriptorSets[currentImage],
                                0, nullptr);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        if (curScene == 0 && (glfwGetKey(window, GLFW_KEY_K) || (present && buttons[1] == GLFW_PRESS)) && !isPopupShown) {
            ...
            for (int i = 1; i < numAssets && notFound; i++) {
                ...
                if (i != FRAMES && dist > 1 && dist <= 5) {
                    cw = (int) glm::sign(YPR.x);
                    float orientation = glm::abs(YPR.x);
                    orientation = orientation - 6 * numCircles;
                    if (checkOrientation(AssetVector[i].pos.x, AssetVector[i].pos.z, camPos.x, camPos.z, orientation,
                                         cw)) {
                        isPopupShown = true;
        ...
        if (isPopupShown && curScene == 0) {
            if (time - debounce > 0.99) {
                float dist = glm::abs(camPos.x - AssetVector[nearestObject].pos.x) +
                             glm::abs(camPos.z - AssetVector[nearestObject].pos.z);
                if (dist > 6) {
                    isPopupShown = false;
                    nearestObject = -1;
                    ...
                    float orientation = glm::abs(YPR.x);
                    orientation = orientation - 6 * numCircles;
                    if (!checkOrientation(AssetVector[nearestObject].pos.x, AssetVector[nearestObject].pos.z, camPos.x,
                                          camPos.z, orientation, cw)) {
                        isPopupShown = false;
                        nearestObject = -1;
        ...
        if ((glfwGetKey(window, GLFW_KEY_SPACE)|| (present && buttons[3] == GLFW_PRESS))) {
            if (time - debounce > 0.33) {
                ...
                if (curScene == 0) {
                    door_dist =
                            glm::abs(camPos.x - AssetVector[DOOR].pos.x) + glm::abs(camPos.z - AssetVector[DOOR].pos.z);
                } else {
                    if (curScene == 1)
                        door_dist = glm::abs(camPos.x - AssetVector2[DOOR2].pos.x) +
                                    glm::abs(camPos.z - AssetVector2[DOOR2].pos.z);
                }
                ...
                if (door_dist <= 5) {
                    ...
                    orientation = orientation - 6 * numCircles;
                    if (curScene == 0) {
                        if (checkOrientation(AssetVector[DOOR].pos.x, AssetVector[DOOR].pos.z, camPos.x, camPos.z,
                                             orientation, cw)) {
                        ...
                        if (curScene == 1) {
                            if (checkOrientation(AssetVector2[DOOR2].pos.x, AssetVector2[DOOR2].pos.z, camPos.x,
                                                 camPos.z, orientation, cw)) {
    ...
    bool canStepPoint(int ind, float x, float y) {
        ...
        int pixX = round(fmax(0.0f, fmin(museumMaps[ind].museumMapWidth - 1,
                                         ((x + map_w[ind] / 2 - map_cx[ind]) * museumMaps[ind].museumMapWidth /
                                          map_w[ind])))); //20
        int pixY = round(fmax(0.0f, fmin(museumMaps[ind].museumMapHeight - 1,
                                         ((y + map_h[ind] / 2 - map_cy[ind]) * museumMaps[ind].museumMapHeight /
                                          map_h[ind])))); //20
